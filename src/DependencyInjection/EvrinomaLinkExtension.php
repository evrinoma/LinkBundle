...
class EvrinomaLinkExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\LinkBundle\Entity';
    public const MODEL = 'Evrinoma\LinkBundle\Model';
    public const ENTITY_FACTORY_LINK = LinkFactory::class;
    public const ENTITY_BASE_LINK = BaseLink::class;
    public const DTO_BASE_LINK = LinkApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_LINK !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, LinkPass::LINK_CONSTRAINT)   :
                    $definition->addTag(LinkPass::LINK_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([LinkCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([LinkQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaLinkBundle::BUNDLE;
    }
}
